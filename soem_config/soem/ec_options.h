/* ec_options.h - static SOEM 2.0 build options.
 *
 * SOEM normally generates this from include/soem/ec_options.h.in via CMake.
 * For a pure-make build we ship it here with SOEM's stock defaults (from
 * SOEM/CMakeLists.txt). Kept in our repo so it travels with git.
 * If you bump the SOEM version, re-check these against the new .in template.
 */
#ifndef _ec_options_
#define _ec_options_
...
/* Timeouts and retries (microseconds) */
...
/* MAC addresses (route differentiation only; not the NIC's MAC).
 * Primary 01:01:01:01:01:01, Secondary 04:04:04:04:04:04 */
